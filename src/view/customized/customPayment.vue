...
export default {
  ...
  methods: {
    ...
    calBalancePay () {
      ...
      if(this.customFeeType == 2){
        url = '/p/musicCustomized/payCustomizeFee';
      }
      this.$http({
        ...
        data: this.$http.adornData({
          payType: 0,
          customizeNo: (this.customFeeType == 1 && this.assessmentType == 1) ? '' : this.customizeNo
        })
      }).then(({ data }) => {
        sessionStorage.setItem("customOrderNo", data.orderNo);
        this.$toast.clear()
        if(this.customFeeType == 1){
    ...
    wexinPay (paydata, cb, errorCb) {
      ...
      this.$http({
        ...
        method: 'post',
        data: process.env.HOST + '/customPayment'
      }).then(({ data }) => {
        // 下面要发起微信支付了
        wx.config({
