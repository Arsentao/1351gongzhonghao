  ...
  <div class="special-edition"
    ...
    <ul class="article-list">
      <router-link tag="li"
        ...
        <i class="top-icon"
           v-show="item.isTop === 1"
           :style="{'display:none':item.active === true}"></i>
        <div class="item-content clearfix">
          ...
          <span class="add-friend"
                ...
                v-if="isSelfPost">添加好友</span>
        </div>
      </router-link>
    </ul>
    <!-- 页面空提示语 -->
    <div class="empty"
         v-if="searchPostList.length==0">这里还没有帖子，去发布第一篇帖子吧！</div>
    <div class="prompt"
         v-if="totalPage>1">
...
export default {
  ...
  methods: {
    ...
    querySearch () {
      ...
      this.$http({
        url: this.$http.adornUrl('/p/post/listByKeyWord'),
        method: 'GET',
        ...
        this.$toast.clear()
        if (data.current == 1) {
          this.searchPostList = data.records
        } else {
          this.searchPostList = this.searchPostList.concat(data.records)
        }
   ...
   */
    getUserInfo () {
      ...
      if (this.searchPostList[0].userId == userInfo.userId) {
